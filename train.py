...
def train_d(net, data, iteration, total_iterations, label="real"):
    ...
    if label == "real":
        pred, [rec_all, rec_small, rec_part], part, _, _ = net(data)
        ...
        pred, [rec_all, rec_small, rec_part], part, feat_out_big, feat_out_small = net(data)
        ...
        return pred.mean().item(), rec_all, rec_small, rec_part, err_fake, feat_out_big, feat_out_small
...
def train(args):
    ...
    for iteration in tqdm(range(current_iteration, total_iterations + 1)):
        ...
        err_dr, rec_img_all, rec_img_small, rec_img_part, d_pred_real = train_d(netD, real_image,
                                                                                               ...
                                                                                               total_iterations,
                                                                                               label="real")
        err_df, rec_fake_all, rec_fake_small, rec_fake_part, d_pred_fake, feat_out_big, feat_out_small = train_d(netD,
                                                                                                  [fi.detach() for fi in
                                                                                                   fake_images_noise],
                                                                                                   iteration,
                                                                                                   total_iterations,
                                                                                                   label="fake")
        err_df_2, rec_fake_all_2, rec_fake_small_2, rec_fake_part_2, d_pred_fake_2, feat_out_big_2, feat_out_small_2 = train_d(netD,
                                                                                                              [fi.detach() for fi in
                                                                                                               fake_images_noise_2],
                                                                                                               iteration,
                                                                                                               total_iterations,
        ...
        lz_D = (torch.mean(torch.abs(feat_out_big_2 - feat_out_big)) + torch.mean(torch.abs(feat_out_small_2 - feat_out_small))) / (torch.mean(torch.abs(noise_2 - noise)))
        ...
        ## 3. train Generator
        netG.zero_grad()
        pred_g, [_, _, _], _, feat_big, feat_small = netD(fake_images_noise)
        pred_g_2, [_, _, _], _, feat_big_2, feat_small_2 = netD(fake_images_noise_2)
        err_g = -pred_g.mean()
        err_g_2 = -pred_g_2.mean()

        lz_G = (torch.mean(torch.abs(feat_big_2 - feat_big)) + torch.mean(torch.abs(feat_small_2 - feat_small))) / (torch.mean(torch.abs(noise_2 - noise)))
        lz_G = 1 / (lz_G + eps)
